...
# Includes the three agent nodes; mgmt-x is excluded
# (mgmt-x is the monitoring server, not a monitored node)
nagios_hostgroup { 'my-nrpe-servers':
  target  => '/etc/nagios4/conf.d/ppt_hostgroups.cfg',
  alias   => 'NRPE Monitored Servers',
  members => 'db-a.oe2.org.nz,app-a.oe2.org.nz,backup-a.oe2.org.nz',
}
